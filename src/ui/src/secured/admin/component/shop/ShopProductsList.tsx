...
export type ShopProductListProps = {
	shopId: number;
};

export default function ShopProductsList({shopId}: ShopProductListProps) {
	const navigator = useNavigator();
	const restClient = useAdminRestClient();
	const userAlerts = useContext(UserAlertsContext);
	const [data, setData] = useState<Page<ShopProduct>>();
	...
	return (
		<div>
			<div className="pt-2 d-flex gap-2 align-items-center">
				<Button variant="primary" size="sm" onClick={() => navigator.admin.shops.products.add(shopId)}>
					+ Add
				</Button>
			...
			<div className="pt-2">
				<TableWithSelect
					...
					onClick={(item) => navigator.admin.shops.products.detail(item.id)}
					items={data.content}
					hover={true}
					striped={true}
				/>
