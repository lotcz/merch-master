	...
	return (
		<div>
			<div className="pt-2 d-flex gap-2 align-items-center">
				<Button variant="primary" size="sm" onClick={() => navigator.admin.accounts.creators.add(accountId)}>
					+ Add
				</Button>
			</div>
			<div className="pt-2">
				<Table hover={true} striped={true}>
					...
					{data.length === 0 ? (
						...
						data.map((c, index) => {
							return (
								<tr key={index} role="button" onClick={() => navigator.admin.accounts.creators.detail(c.id)}>
									<td>{c.id}</td>
									<td>{c.user.name}</td>
									<td>{c.user.email}</td>
									<td>{c.userState}</td>
