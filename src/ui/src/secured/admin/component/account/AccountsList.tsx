...
export default function AccountsList() {
	...
	const applySearch = useCallback(
		(e: FormEvent) => {
			...
			paging.page = 0;
			navigator.admin.accounts.list(paging);
		},
		...
		<div>
			<div className="pt-2 ps-3">
				<Stack direction="horizontal" gap={2}>
					...
					<Button onClick={() => navigator.admin.accounts.add()} className="text-nowrap">
						+ Add
					</Button>
					<div style={{width: "250px"}}>
						<Form onSubmit={applySearch}>
							<TextInputWithReset
								...
								onReset={() => {
									setSearchInput("");
									navigator.admin.accounts.list(DEFAULT_PAGING);
								}}
							/>
						</Form>
					</div>
			...
			<div className="px-3 gap-3">
				{data === null ? (
					...
					<TableWithSelect
						...
						onPagingChanged={(p) => navigator.admin.accounts.list(p)}
						onClick={(item) => navigator.admin.accounts.detail(item.id)}
						items={data.content}
						hover={true}
						striped={true}
					/>
