...
export default function OrdersList() {
	...
	const applySearch = useCallback(
		(e: FormEvent) => {
			...
			paging.page = 0;
			navigator.admin.shops.list(paging);
		},
					...
					<div style={{width: "250px"}}>
						<Form onSubmit={applySearch}>
							<TextInputWithReset
								...
								onReset={() => {
									setSearchInput("");
									navigator.admin.orders.list(DEFAULT_PAGING);
								}}
							/>
						</Form>
					</div>
			...
			<div className="px-3 gap-3">
				{data === null ? (
					...
					<TableWithSelect
						...
						onPagingChanged={(p) => navigator.admin.orders.list(p)}
						onClick={(item) => navigator.admin.orders.detail(item.id)}
						items={data.content}
						hover={true}
						striped={true}
					/>
