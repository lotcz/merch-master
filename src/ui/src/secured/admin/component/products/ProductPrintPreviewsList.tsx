	...
	return (
		<div>
			<div className="pt-2 d-flex gap-2 align-items-center">
				<Button variant="primary" size="sm" onClick={() => navigator.admin.products.printPreviews.add(productId)}>
					+ Add
				</Button>
			</div>
			<div className="pt-2">
				<Table hover={true} striped={true}>
					...
					{data.length === 0 ? (
						...
						data.map((pt, index) => {
							return (
								<tr key={index} role="button" onClick={() => navigator.admin.products.printPreviews.detail(pt.id)}>
									<td>{pt.id}</td>
									<td>{pt.name}</td>
								</tr>
							);
