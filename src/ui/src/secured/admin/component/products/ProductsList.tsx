...
export default function ProductsList() {
	...
	const applySearch = useCallback(
		(e: FormEvent) => {
			...
			paging.page = 0;
			navigator.admin.products.list(paging);
		},
		...
		<div>
			<div className="pt-2 ps-3">
				<Stack direction="horizontal" gap={2}>
					...
					<Button onClick={() => navigator.admin.products.add()} className="text-nowrap">
						+ Add
					</Button>
					<div style={{width: "250px"}}>
						<Form onSubmit={applySearch} id="topics-search-form">
							<TextInputWithReset
								...
								onReset={() => {
									setSearchInput("");
									navigator.admin.products.list(DEFAULT_PAGING);
								}}
							/>
						</Form>
					</div>
			...
			<div className="px-3 gap-3">
				{data === null ? (
					...
					<TableWithSelect
						...
						onPagingChanged={(p) => navigator.admin.products.list(p)}
						onClick={(item) => navigator.admin.products.detail(item.id)}
						items={data.content}
						hover={true}
						striped={true}
					/>
