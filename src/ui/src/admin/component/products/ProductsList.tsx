...
export default function ProductsList() {
	...
	const applySearch = useCallback(
		(e: FormEvent) => {
			...
			paging.page = 0;
			navigator.products.list(paging);
		},
		...
		<div>
			<div className="pt-2 ps-3">
				<Stack direction="horizontal" gap={2}>
					...
					<Button onClick={() => navigator.products.add()} className="text-nowrap">
						+ Add
					</Button>
					<div style={{width: "250px"}}>
						<Form onSubmit={applySearch} id="topics-search-form">
							<TextInputWithReset
								...
								onReset={() => {
									setSearchInput("");
									navigator.products.list(DEFAULT_PAGING);
								}}
							/>
						</Form>
					</div>
			...
			<div className="px-3 gap-3">
				{data === null ? (
					...
					<TableWithSelect
						...
						onPagingChanged={(p) => navigator.products.list(p)}
						onClick={(item) => navigator.products.detail(item.id)}
						items={data.content}
						hover={true}
						striped={true}
					/>
