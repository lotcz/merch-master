...
export default function DesignDetail() {
	...
	const onChanged = useCallback((design: DesignPayload) => {
		setData(design);
		setChanged(true);
	}, []);
	...
	const reload = useCallback(() => {
		...
		if (id) {
			...
			// create new design
			designerClient
				.createNewDesign(Number(productId))
				.then(setData)
				.catch((e: Error) => userAlerts.err(e));
		}
	...
	const deleteDesign = useCallback(() => {
		...
		confirmDialog.confirm("Confirm", "Really delete this design?", () => {
			setDeleting(true);
			restClient.designs
		...
		<div>
			<div className="p-2">
				<Stack direction="horizontal" gap={2}>
					...
					<DeleteButton loading={deleting} disabled={!data.design.id} onClick={deleteDesign}>
						Delete
					</DeleteButton>
				</Stack>
			</div>
