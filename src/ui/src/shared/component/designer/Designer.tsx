...
export default function Designer({ design, saving, changed, readOnly, admin, onChange, onError, onFinished, onCancel }: DesignerParams) {
	...
	return (
		<div className="designer">
			{selectedPreview ? (
				<div className="preview-full card designer-shadow" onClick={() => setSelectedPreview(undefined)}>
					<div className="d-flex justify-content-end cursor-pointer">
						<Button variant="link" size="sm" onClick={() => setSelectedPreview(undefined)}>
							Zavřít náhled
						</Button>
					</div>
					...
					<Row>
						...
						<Col md={5} lg={6} xl={8} className="mt-2 mt-md-0">
							<div className="designer-shadow designer-main">
								...
								>
									{printType ? (
										printType.zones.map((zone, index) => <Tab key={index} title={zone.name} eventKey={String(zone.id)} />)
									) : (
										<Spinner />
									)}
								</Tabs>
							...
							<div className="previews-section card designer-shadow p-2">
								<div ref={previewAreaRef} className="previews">
									<div className="preview-container">
										{printType.previews.map((preview, index) => (
											<DesignerPreview
												key={index}
												preview={preview}
												design={design}
								...
								<div className="preview-action">
									{readOnly ? (
										...
										<LoadingButton loading={saving} disabled={!changed} size="lg" variant="success" onClick={onFinished}>
											Uložit
										</LoadingButton>
									)}
								</div>
