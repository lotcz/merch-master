...
export default function DesignerPreviewZone({
	...
	const files = useMemo(() => design.files.filter((f) => f.printZoneId === previewZone.printZoneId), [design, previewZone]);

	const zone = useMemo(() => zones.find((z) => z.id === previewZone.printZoneId), [zones, previewZone]);
	...
	const loadFileUrls = useCallback(() => {
		...
		const promises: Array<Promise<FileImageUrl>> = names.map(
			(n): Promise<FileImageUrl> =>
				imagezClient
					.getResizedUrl(n, "Fit", designerWidth, designerHeight, undefined, undefined, undefined, true)
					.then((url): FileImageUrl => {
						return {
							imageName: n,
							imageUrl: url,
						};
		...
		Promise.all(promises).then((fileImages) => {
	...
	const loadFileImages = useCallback(() => {
		...
		fileUrls.forEach((url, name) => {
			const promise = new Promise<FileImage>((resolve) => {
	...
	return previewZone.useCylinderEffect ? (
		<DesignerPreviewCylinderZone zoneImage={canvasAsUrl} previewZone={previewZone} previewScale={previewScale} width={width} height={height} />
	) : (
		<DesignerPreviewFlatZone zoneImage={canvasAsUrl} previewZone={previewZone} previewScale={previewScale} width={width} height={height} />
	);
}
