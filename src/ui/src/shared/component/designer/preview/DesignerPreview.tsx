...
export default function DesignerPreview({ design, preview, productZones, maxHeight, maxWidth, onError, onClick }: DesignerPreviewParams) {
	const scale = useMemo(() => {
		return ImageUtil.imageFitScale(preview.printPreview.imageWidthPx, preview.printPreview.imageHeightPx, maxWidth, maxHeight);
	...
	return (
		<div className={`designer-preview-wrapper ${onClick ? "cursor-pointer" : ""}`} onClick={() => (onClick ? onClick() : null)}>
			<div className="text-center">
				<strong>{preview.printPreview.name}</strong>
			</div>
			<div
			...
			>
				...
				{preview.zones.map((previewZone, index) => (
					<DesignerPreviewZone
						key={index}
						design={design}
						zones={productZones}
				...
				{preview.printPreview.foregroundName && (
