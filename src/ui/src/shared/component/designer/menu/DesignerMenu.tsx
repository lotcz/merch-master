	...
	return (
		<Form>
			<Form.Group>
				...
				<div>{product ? <strong>{product.name}</strong> : <Spinner size="sm" />}</div>
			</Form.Group>
			<Form.Group>
				<Form.Label title="Barva">Barva</Form.Label>
				{colors ? (
					<ColorSelectId id={design.design.productColorId} colors={colors} readOnly={readOnly} onSelected={onColorChange} />
				) : (
					<Spinner />
				)}
			</Form.Group>
			...
			{product && selectedZone && files && (
				<Accordion activeKey={String(selectedFile?.id)} alwaysOpen={false}>
					{files.map((file, index) => (
						<Accordion.Item key={index} eventKey={String(file.id)} onSelect={() => onFileSelected(file)}>
