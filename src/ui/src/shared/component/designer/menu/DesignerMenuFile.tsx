...
export default function DesignerMenuFile({
	...
	const guessBackground: () => Promise<ImagezColorPayload | null> = useCallback((): Promise<ImagezColorPayload | null> => {
		setGuessing(true);
		return imagezClient.guessBackgroundColor(selectedFile.imageName).then((c) => {
			setGuessing(false);
			return c;
		});
	...
	return (
		<div>
			...
			{StringUtil.notBlank(selectedFile.removeBackgroundColor) && (
				<div>
					<ResetableRange
			...
			<div className="text-center mt-2">
				<IconButton variant="danger" size="sm" icon={<BsTrash />} onClick={onFileDeleted}>
					Odstranit
				</IconButton>
			</div>
		</div>
