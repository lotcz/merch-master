...
export class DesignerRestClient extends RestClient {
	...
	createNewDesign(productId: number): Promise<DesignPayload> {
		return this.loadPrintTypesByProduct(productId).then((printTypes) => {
			if (printTypes.length === 0) {
				throw new Error("NO PRINT TYPES!");
			}
			const printTypeId = Number(printTypes[0].id);
			return this.loadColorsByProduct(productId).then((colors) => {
				if (colors.length === 0) {
...
export const DesignerRestClientContext = createContext(new DesignerRestClient());
